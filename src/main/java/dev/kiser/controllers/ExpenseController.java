...
public class ExpenseController {
    ...
    private String isManager(String eid, String status)
            throws OperationNotPossible, EmployeeNotFoundException, ExpenseException {

        Set<Expense> expenses;

        // get all expenses if query params do not exist
        if (status.equals("NONE")) {
            if (Integer.parseInt(eid) != -1) {
                expenses = this.expenseService.getExpensesByEmployee(Integer.parseInt(eid));
            } else {
